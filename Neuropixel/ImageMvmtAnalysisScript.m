function [] = ImageMvmtAnalysisScript()

vid_path = 'X:\CorinnaNeuropixSleepData\data\r5n5\20201023_02\corrected1fpsFrames\';
%vid_path = 'X:\CorinnaNeuropixSleepData\data\AninditasBird\camera_redred-Zf4\';



imageNames = dir(fullfile(vid_path, '*.png'));
%imageNames = dir(fullfile(vid_path, '*.jpg'));
...
resamp = resample(r_dif, 2,1);

v = r_dif;
xq = 1.5:numel(r_dif)+1; % for 1 frame ever 2 s

v = r_dif;
xq = 1.2:0.2:numel(r_dif)+1; % for 1 frame every 5 s
...
subplot(2, 1, 1)
plot(mic, 'linestyle', 'none', 'marker', '.')
