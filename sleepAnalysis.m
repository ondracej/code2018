classdef sleepAnalysis < handle
    ...
    methods
        ...
        function data=getDelta2BetaRatio(obj,varargin)
            ...
            deltaBandCutoff = 8; % 4;
            betaBandLowCutoff = 10; % 10;
            betaBandHighCutoff = 40; % 40;
        ...
        function [data]=getDelta2BetaAC(obj,varargin)
            ...
            if ~isempty(pStartSleep)
                ...
                tEndSleep=[];
                pSleepSlidingAC=[];
                filteredSlidingPeriod=[];
                edgeSamples=[];
                filteredSlidingPeriod=[];
                tFilteredSlidingPeriod=[];
            end
            ...
            if plotSpectralBands
                ...
                if savePlots
                    ...
                    %fileName=[obj.currentPlotFolder filesep 'spectralBands_ch' num2str(parFreqBandDetection.ch) '_t' num2str(parFreqBandDetection.tStart) '_w' num2str(parFreqBandDetection.win)];
                      fileName=[obj.currentPlotFolder filesep 'spectralBands_ch' num2str(parFreqBandDetection.ch) '_t' num2str(round(parFreqBandDetection.tStart)) '_w' num2str(round(parFreqBandDetection.win))];
                    print(fileName,'-djpeg',['-r' num2str(obj.figResJPG)]);
                    print(fileName,'-depsc',['-r' num2str(obj.figResEPS)]);
                    if printLocalCopy
        ...
        %% getFileNames
        function obj=getFileNames(obj,varargin)
            %obj.checkFileRecording;
        ...
        function [obj]=getExcelData(obj,excelRecordingDataFileName,additionalExcelFieldNames)
            ...
            if nargin==3
                obj.relevantFields=[obj.relevantFields additionalExcelFieldNames];
            end
            ...
            xlsFieldNames=excelTxt(1,:);
            maxRow=size(excelTxt,1);
            
            for i=1:numel(obj.relevantFields)
                pFieldInExcel=find(strcmp(xlsFieldNames,obj.relevantFields{i}));
                if ~isempty(pFieldInExcel)
                    obj.par.(obj.relevantFields{i})=excelRaw(obj.titleLine+1:maxRow,pFieldInExcel+obj.startCol-1);
                else
                    obj.par.(obj.relevantFields{i})=[];
                end
            end
            obj.par.Num=cell2mat(obj.par.Num);
            obj.nSessions=numel(obj.par.(obj.relevantFields{1}));
        end
        
        %% setCurrentRecording
        function [obj]=setCurrentRecording(obj,recNumber) %if recNumber is negative loads all recording related files but not recording object
             if nargin==2
                 if recNumber>0
                    pRec=find(obj.par.Num==recNumber);
                    obj.currentDataObj=NLRecording(obj.par.CheetahFolder{pRec});
                 else
                     pRec=find(obj.par.Num==-recNumber);
                 end
                 obj.currentPRec=pRec;
                 CheetahStartChar = strfind(obj.par.CheetahFolder{obj.currentPRec},'_cheetah');
                 if isempty(CheetahStartChar) %if the word _cheetah does not exist in the name
                     CheetahStartChar=numel(obj.par.CheetahFolder{obj.currentPRec})+1;
                 end
                 obj.currentAnalysisFolder=[obj.par.CheetahFolder{obj.currentPRec}(1:CheetahStartChar-1) '_analysis'];
                 obj.currentPlotFolder=[obj.par.CheetahFolder{obj.currentPRec}(1:CheetahStartChar-1) '_plots'];
                 obj.currentVideosFolder=[obj.par.CheetahFolder{obj.currentPRec}(1:CheetahStartChar-1) '_videos'];
                 ...
                 fprintf('Current exp. set to: %s\n',obj.par.CheetahFolder{pRec});
             elseif nargin==1
                 disp('Not enough inputs, specify a recording number');
                 return;
             end
        end
        ...
        function [obj]=getFilters(obj,Fs)
            ...
            obj.filt.F.downSamplingFactor=128;
            obj.filt.F=obj.filt.F.designDownSample;
            obj.filt.F.padding=true;
            obj.filt.FFs=obj.filt.F.filteredSamplingFrequency;
            
            
            obj.filt.F2=filterData(Fs);
            obj.filt.F2.downSamplingFactor=256;
            obj.filt.F2=obj.filt.F2.designDownSample;
            obj.filt.F2.padding=true;
            obj.filt.FFs=obj.filt.F2.filteredSamplingFrequency;
            
            obj.filt.DS4Hz=filterData(Fs);
            obj.filt.DS4Hz.downSamplingFactor=256;
            %obj.filt.DS4Hz.downSamplingFactor=128;
            ...
            obj.filt.FJLB=filterData(Fs);
            obj.filt.FJLB.highPassCutoff=1;
            obj.filt.FJLB.lowPassCutoff=2000;
            obj.filt.FJLB.filterDesign='butter';
            obj.filt.FJLB=obj.filt.FJLB.designBandPass;
            obj.filt.FJLB.padding=true;
            
            obj.filt.Femg=filterData(Fs);
            obj.filt.Femg.highPassCutoff=10;
            obj.filt.Femg.lowPassCutoff=500;
            obj.filt.Femg.filterDesign='butter';
            obj.filt.Femg=obj.filt.Femg.designBandPass;
            obj.filt.Femg.padding=true;
            
            
            obj.filt.FSpk=filterData(Fs);
            %obj.filt.FSpk.highPassCutoff=600;
            obj.filt.FSpk.highPassCutoff=300;
            obj.filt.FSpk.lowPassCutoff=2000;
            obj.filt.FSpk.filterDesign='butter';
            obj.filt.FSpk=obj.filt.FSpk.designBandPass;
            obj.filt.FSpk.padding=true;
