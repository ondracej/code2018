function [] = detectSWRsJO_2021_FrankMethod_v2()
...
%% For estimating the artifact rejection
%{
tic
[data, timestamps, info] = load_open_ephys_data([SessionDir dataRecordingObj.channelFiles{1}]);
toc
...
nCycles = numel(TOn);
    ...
    nTestSegments  = round(nCycles*.25);

rng(1); % for reproducibiity
allSegsRand = randperm(nCycles);
...
for r = 1:4 
    ...
    if r == 1
        pCycle=sort(allSegsRand(1:nTestSegments*r));
    elseif r == 4
        pCycle=sort(allSegsRand(nTestSegments*(r-1)+1:nCycles));
    else
        pCycle=sort(allSegsRand(nTestSegments*(r-1)+1:nTestSegments*r));
    end
    
    pcycles{r} = pCycle;
    allDataNoArtifacts{r} =  getDataSegs(pCycle, ch, TOn, seg_ms, fobj, dataRecordingObj, artifactThresh_pos, artifactThresh_neg);
    
    nonemptyInds(r) = numel(find(cellfun(@(x) ~isempty(x), allDataNoArtifacts{r})));
    
    allNonempyInds = sum(nonemptyInds(:));
    
    if allNonempyInds >= round(nCycles*.25)
        disp('')
        break
    end
...
allDataSegs_NoArtifacts = [];
for o = 1:numel(allDataNoArtifacts)
    
    thisSetDataSegs = allDataNoArtifacts{o};
    nonemptyInds = find(cellfun(@(x) ~isempty(x), thisSetDataSegs ));    
    allDataSegs_NoArtifacts = [allDataSegs_NoArtifacts thisSetDataSegs(nonemptyInds)];
    
end

if numel(allDataSegs_NoArtifacts) > round(nCycles*.25)
    allDataSegs_NoArtifacts = allDataSegs_NoArtifacts(1: round(nCycles*.25));
end


powerTrace_rip = []; DataSeg_SW = [];
    for i = 1:numel(allDataSegs_NoArtifacts)
        
    DataSeg_BP_N = allDataSegs_NoArtifacts{i};
    %figure(104); plot(squeeze(DataSeg_BP_N(1,:)))  
    ...
    %  lets rather pick one chanel and do the SW detection from that
     ...
     DataSeg_SW{i} = squeeze(fobj.filt.SW2.getFilteredData(DataSeg_BP_N)); % downsampled data --> SW Filter original
...
[minVal, SW_ind] = min(min(allCHans, [], 2));

SW_chan = allCHans(SW_ind,:);

[Z_sample_sw,mean_sample_sw,std_sample_sw]= zscore(SW_chan);
mean_zscore_sw_sample = mean(Z_sample_sw);

%figure(323)
%plot(-Z_sample_sw(1:20000));
...
for k=1:numel(TOn)-1
    ...
    if artifactPosCheck || artifactNegCheck
       % disp('Artifact Detected')
        ...
        diffsArtInds = diff(allArtInds);
        %figure; plot(diffsArtInds);
        
        crossings = find(diffsArtInds > 1);
        
        artifactPadding = [];
        durToPad_ms = 1000;
        durToPad_samp = (durToPad_ms/1000)*Fss;
        for a = 1:numel(crossings)
            
            thisInd = allArtInds(crossings(a));
            thisIndNext = allArtInds(crossings(a)+1);
            
            artifactPadding(a,:) = [thisInd:1:thisInd+durToPad_samp thisIndNext-durToPad_samp:1:thisIndNext];
        end
            
        newInds = artifactPadding(:);
        firstLastInds = allArtInds(1)-durToPad_samp:1:allArtInds(1);
        
        finalArtInds = unique(sort([allArtInds newInds' firstLastInds]));
        neg2Inds = find(finalArtInds <=0);
        finalArtInds(neg2Inds) = [];
        
        pos2Inds = find(finalArtInds >numel(sumData));
        finalArtInds(pos2Inds) = [];
    ...
    zscore_rip(finalArtInds) = 1e-15;
   
    % SW
    DataSeg_SW = squeeze(fobj.filt.SW2.getFilteredData(DataSeg_BP_N)); % down sampled  --> sharp wave
    DataSeg_SW_negChan =  DataSeg_SW(SW_ind,:);
    
    zscore_SW = (DataSeg_SW_negChan - mean_sample_sw) ./ std_sample_sw;
    zscore_SW(finalArtInds) = 1e-15;
    
    %{
    figure(331); clf
    subplot(2, 1, 1)
    plot(DataSeg_SW_negChan)
    subplot(2, 1, 2)
    plot(zscore_SW)
    %}
    ...
    hold on
    yvals = zeros(1, numel(finalArtInds));
    plot(finalArtInds, yvals, 'k.')
    ylim([-500 400])
    title('BP Raw Data')
    ...
    %ylim([0 5e5])
    title('Raw Data - SW Filter')
     ylim([-500 250])
    
    subplot(5, 1, 5)
    plot(-zscore_SW); axis tight
    ylim([-4 4])
    title('Negative SW Z-Score')
    %%  Ripple Detections
    ...
    [pks_sw,locs_sw,w_sw,p_sw] = findpeaks(-zscore_SW, 'minPeakProminence', peakProm, 'MinPeakHeight', thresh, 'MinPeakWidth',  midWidth_samples, 'MaxPeakWidth', maxWidth_samples, 'MinPeakDistance', peakDistance_sample, 'WidthReference', 'Halfheight' );
    %[pks_sw,locs_sw,w_sw,p_sw] = findpeaks(-zscore_SW, 'MinPeakHeight', thresh, 'MinPeakWidth',  midWidth_samples, 'MaxPeakWidth', maxWidth_samples, 'MinPeakDistance', peakDistance_sample, 'WidthReference', 'Halfheight' );
    subplot(5, 1, 5)
    hold on
    ...
    if nPeaks_sw > 0
        ...
        for j = 1:nPeaks_sw
            ...
            %{
                ...
                plot(ROI_l)
                hold on
                line([0 searchWin_samples], [mean_zscore_sw_sample mean_zscore_sw_sample],'color','k')
                hold on
                line([searchWin_samples-maxVal searchWin_samples-maxVal], [-2 2],'color','k')
            %}
            ...
            SwROffset = thisPeak+minVal;
                ...
                %{
                    ...
                    line([0 searchWin_samples], [mean_zscore_sw_sample mean_zscore_sw_sample],'color','k')
                    hold on
                    line([0+minVal 0+minVal], [-2 2],'color','k')
                %}
    ...
    allArtifacts_abs{k} = finalArtInds+TOn(k)/1000*Fss;
end
...
function allDataNoArtifacts =  getDataSegs(pCycle, ch, TOn, seg_ms, fobj, dataRecordingObj, artifactThresh_pos, artifactThresh_neg)
...
for i=1:numel(pCycle)
    ...
    allDataNoArtifacts{i} = DataSeg_BP_N;
