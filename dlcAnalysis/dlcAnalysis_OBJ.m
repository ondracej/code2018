classdef dlcAnalysis_OBJ < handle
    ...
    methods
        ...
        function obj = getVideoInfo(obj,analysisDir, VidToAnalyze)
            ...
            [pathstr,name,ext] = fileparts(VidToAnalyze);
            searchString = ['*' name '*'];
            
            files = dir(fullfile(analysisDir, searchString));
            nFiles = numel(files);
            ...
            plot_path = [analysisDir name '_Plots' dirD];
            
            obj.PATH.vidPath = origVideo_path;
            obj.PATH.dataPath = data_path;
            obj.PATH.plotPath = plot_path;
        ...
        function [obj] = plotTrajectories(obj)
          ...
          cols = {'r', 'c', 'g', 'b', 'y', 'm',  'w', 'k'}; 
            ...
            for j = 1:nChoices
                ...
                plot(coords_X, coords_Y, 'color', cols{j}, 'marker', '.', 'markersize', 20, 'linestyle', '-')
                trackedText = [trackedText '-' thisTrackedObject];
            end
            
            figure(figH)
            axis off
            annotation(figH,'textbox',[0.1 0.96 0.46 0.028],'String',[obj.VID.vidName '-' trackedText],'LineStyle','none','FitBoxToText','off', 'fontsize', 12);
            disp('Printing Plot')
         ...
         function [obj] = plotTrajectories_with_likelihood(obj, likelihood_cutoff)
          ...
          cols = {'r', 'c', 'g', 'b', 'y', 'm',  'w', 'k'}; 
            ...
            for j = 1:nChoices
                ...
                hold on
                % plot(coords_X, coords_Y, 'color', cols{j}, 'marker', '.', 'markersize', 10, 'linestyle', 'none')
                plot(coords_X(HL_inds), coords_Y(HL_inds), 'color', cols{j}, 'marker', '.', 'markersize', 20, 'linestyle', '-')
                trackedText = [trackedText ' | ' thisTrackedObject ' : n=' num2str(n_inds) '/' num2str(numel(likelihood))];
            end
            
            figure(figH)
            axis off
            annotation(figH,'textbox',[0.1 0.96 0.7 0.028],'String',[obj.VID.vidName ', LH = ' num2str(likelihood_cutoff) trackedText],'LineStyle','none','FitBoxToText','off', 'fontsize', 12);
            disp('Printing Plot')
            %set(0, 'CurrentFigure', figH)
            
            plotPath = obj.PATH.plotPath;
            saveName = [plotPath obj.VID.vidName(1:end-4) trackedText_save '_tracked-LH'];
            plotpos = [0 0 70 40]; % keep this so arena dims look ok
            print_in_A4(0, saveName, '-djpeg', 0, plotpos);
    ...
    methods (Hidden)
        %class constructor
        function obj = dlcAnalysis_OBJ(analysisDir, VidToAnalyze)
            
            addpath(genpath(analysisDir)) 
           
            obj = getVideoInfo(obj, analysisDir, VidToAnalyze);
