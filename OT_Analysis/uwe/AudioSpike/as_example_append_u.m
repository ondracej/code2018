...
AudioSpike.Settings.Level           = [30]';%[75 85] %ausklammern wenn nicht gewünscht!


% if  OTHER parameters to be appended, then we have create them!!

NewOtherParameters = 0;% other than level!!!

if (NewOtherParameters)
    % 2. create stimuli to be appended: NOTE: this loop is identical to the
    % creation loop in sw_example_create.m
    % - new frequency
    freqs  = [30000:10000:80000];% [750]
...
% stimulus data
audiospike_createmeas(AudioSpike, 'as_example_append_u.xml');
...
system('start audiospike.exe command=append file=as_example_append_u.xml');