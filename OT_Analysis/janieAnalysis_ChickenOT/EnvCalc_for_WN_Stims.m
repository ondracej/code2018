function [] = EnvCalc_for_WN_Stims(experiment, recSession, NeuronName)
dbstop if error
if nargin <3
...
FigSaveDir = '/media/dlc/Data8TB/TUM/OT/Figs/EnvAnalysis-WN/';
...
for j = 1:nRows
    for k = 1:nCols
        ...
        if k == 1 && j == 1
            ...
            ylim([0 1])
            title([NeuronName ': WN Signal Envelope and PSTH'])
            xlabel('Time [ms]')
...
%             ylim([0 1])
%             title([NeuronName ': Right HRTF Signal Envelope and PSTH'])
        end
...
subplot(3, 1, 3)
boxplot([ccL_r_sig ]', 'whisker', 0, 'symbol', 'k.', 'outliersize', 4,  'jitter', 0, 'colors', [0 0 0], 'labels', {'WN'})
hold on

xes = ones(1, numel(ccL_r_sig)); 
%plot(xes, ccL_r_sig, 'k.', 'linestyle', 'none')
scatter(xes, ccL_r_sig, 'k.', 'jitter','on', 'jitterAmount', 0.02);

title('Significant correlation between WN envelope and PSTH')
ylim([-.6 .6])
...
set(gca, 'xticklabel', xtickabs )
title('Mean cross-correlation: WN stim envelope and PSTH')
xlabel('Lag [ms]')
