...
AVG.Params.samplingFrequency=20000;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function initializeViewer()
        ...
        AVG.Params.Bin=1/AVG.Params.samplingFrequency*1000; %convert from sampling freq [Hz] to time [ms]
        ...
        %construct filter object
        %AVG.filterObj=filterData(AVG.Params.samplingFrequency); %filter constructor
        AVG.filterObj.samplingFrequency=AVG.Params.samplingFrequency;
        set(AVG.hFilter.hSamplingFrequencyEdit,'string',num2str(AVG.Params.samplingFrequency));
        AVG.Params.paddingSamples=AVG.Params.filterPadding/1000*AVG.Params.samplingFrequency;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function CallbackDataTypeSelection(hObj,event)
        ...
        if AVG.hTrigger.hGetTrigFromRawFiles
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function CallbackChManualChannelSelectPush(hObj,event)
        ...
        AVG.manualChannelSelectionFigure.VBox=uiextras.VBox('Parent',AVG.manualChannelSelectionFigure.hFigure, 'Spacing',4, 'Padding',4);
        AVG.manualChannelSelectionFigure.hChannelGrid = uiextras.Grid('Parent',AVG.manualChannelSelectionFigure.VBox, 'Spacing',5, 'Padding',10);
        ...
        set(AVG.manualChannelSelectionFigure.hChannelGrid,'ColumnSizes',-1*ones(1,ceil(nActiveChannels/channelsPerColumn)),'RowSizes', -1*ones(1,channelsPerColumn));
        set(AVG.manualChannelSelectionFigure.VBox, 'Sizes',[-1 30]);
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function designFilter()
        ...
        if ~strcmp(AVG.filterObj.currentDesignedFilter,'bandstop')
            if AVG.Params.samplingFrequency<2*AVG.filterObj.lowPassPassCutoff
                AVG.filterObj.lowPassStopCutoff=AVG.Params.samplingFrequency/2;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function createAVGUI(figH)
        ...
        %uiextras.set(AVG.hMainFigure.hFigure, 'DefaultBoxPanelTitleColor',[0.7 1.0 0.7]);
        ...
        AVG.hMainFigure.hMainWindow = uiextras.HBox('Parent',AVG.hMainFigure.hFigure, 'Spacing',4);
        AVG.hMainFigure.hLeftBox = uiextras.VBox('Parent',AVG.hMainFigure.hMainWindow, 'Spacing',4, 'Padding',4);
        AVG.hMainFigure.hMidGrid = uiextras.Grid('Parent',AVG.hMainFigure.hMainWindow, 'Spacing',4, 'Padding',7);
        AVG.hMainFigure.hRightBox = uiextras.VBox('Parent',AVG.hMainFigure.hMainWindow, 'Spacing',4, 'Padding',4);
        set(AVG.hMainFigure.hMainWindow, 'Sizes',[-2 -8 -2]);
        
        % Set left box
        AVG.hGen.General = uiextras.Panel('Parent',AVG.hMainFigure.hLeftBox, 'Title','General');
        AVG.hNav.MainPanel = uiextras.Panel('Parent',AVG.hMainFigure.hLeftBox, 'Title','Navigation');
        AVG.hCh.ChannelSelectionPanel = uiextras.Panel('Parent',AVG.hMainFigure.hLeftBox, 'Title','Channel selection');
        AVG.hPlotProp.hMainPanel = uiextras.Panel('Parent',AVG.hMainFigure.hLeftBox, 'Title','Plot specific properties');
        set(AVG.hMainFigure.hLeftBox, 'Sizes',[-1 -2 -2 -3]);
        
        % Set mid box - main plot
        AVG.hMainFigure.dummyEmptyBoxes(1)=uiextras.Empty('Parent', AVG.hMainFigure.hMidGrid,'Visible','off');
        AVG.hMainFigure.dummyEmptyBoxes(2)=uiextras.Empty('Parent', AVG.hMainFigure.hMidGrid,'Visible','off');
        AVG.hMainFigure.dummyEmptyBoxes(3)=uiextras.Empty('Parent', AVG.hMainFigure.hMidGrid,'Visible','off');
        ...
        AVG.hMainFigure.hMainAxis=axes('Parent', AVG.hMainFigure.hMidGrid, 'ActivePositionProperty','Position', 'FontSize',8,'NextPlot','add');
        AVG.hMainFigure.dummyEmptyBoxes(4)=uiextras.Empty('Parent', AVG.hMainFigure.hMidGrid,'Visible','off');
        set(AVG.hMainFigure.hMidGrid,'ColumnSizes',[45 -1],'RowSizes', [AVG.Params.scrollBarPixels -1 35]);
        
        % Set right box
        AVG.hProcessing.hMainPanel = uiextras.TabPanel('Parent',AVG.hMainFigure.hRightBox, 'Padding',2, 'TabSize',90);
        AVG.hFilter.hMainPanel=uiextras.Panel('Parent',AVG.hProcessing.hMainPanel, 'Padding',4);
        AVG.hAnalysis.hMainPanel=uiextras.Panel('Parent',AVG.hProcessing.hMainPanel, 'Padding',4);
        set(AVG.hProcessing.hMainPanel,'TabNames',{'Filter','Analysis'}, 'SelectedChild',2);
        
        AVG.hTrigger.hMainTriggerPanel = uiextras.Panel('Parent',AVG.hMainFigure.hRightBox, 'Title','Triggers');
        set(AVG.hMainFigure.hRightBox, 'Sizes',[-1,-1]);
        %% construch general GUI
        AVG.hGen.mainVBox=uiextras.VBox('Parent', AVG.hGen.General, 'Padding', 2, 'Spacing', 5);
        ...
        AVG.hGen.generalGrid=uiextras.Grid('Parent', AVG.hGen.mainVBox, 'Padding', 2, 'Spacing', 5);
        ...
        set(AVG.hGen.generalGrid, 'ColumnSizes',-1,'RowSizes', -1 );
        set(AVG.hGen.mainVBox,'Sizes',[-1,-3]);
        %% Construct Navigation GUI
        AVG.hNav.mainBox=uiextras.VBox('Parent', AVG.hNav.MainPanel, 'Padding', 5, 'Spacing', 10);
        
        AVG.hNav.timeControlBox=uiextras.HBox('Parent', AVG.hNav.mainBox, 'Padding', 5, 'Spacing', 10);
        ...
        AVG.hNav.ZoomBox=uiextras.VBox('Parent', AVG.hNav.timeControlBox, 'Padding', 5, 'Spacing', 10);
        ...
        set(AVG.hNav.timeControlBox, 'Sizes',[-2,-3,-4,-3,-2]);
        
        AVG.hNav.timeControlGrid=uiextras.Grid('Parent', AVG.hNav.mainBox, 'Padding', 5, 'Spacing', 10);
        ...
        set(AVG.hNav.timeControlGrid, 'ColumnSizes',[-2 -1],'RowSizes', [-1 -1] );
        
        set(AVG.hNav.mainBox, 'Sizes',[-3,-2]);
        %% Construct channel GUI
        AVG.hCh.mainBox=uiextras.VBox('Parent', AVG.hCh.ChannelSelectionPanel, 'Padding', 5, 'Spacing', 10);
        ...
        %AVG.hCh.empty1=uiextras.Empty('Parent', AVG.hCh.mainBox);
        
        set(AVG.hCh.mainBox, 'Sizes',[-1,-1,-1,-1,-1]);
        %% Construct filter GUI
        % set filter panel
        %fileds are arranged as a N x 2 grid of text and input
        
        AVG.hFilter.hMainVBox= uiextras.VBox( 'Parent', AVG.hFilter.hMainPanel, 'Spacing',4, 'Padding',4);
        AVG.hFilter.hMainGrid= uiextras.Grid( 'Parent', AVG.hFilter.hMainVBox, 'Spacing',10 );
        ...
        AVG.hFilter.hSamplingFrequencyEdit=uicontrol('Parent', AVG.hFilter.hMainGrid, 'Callback',{@CallbackFilterSamplingFrequencyEdit}, 'Style','edit', 'String',num2str(AVG.Params.samplingFrequency));
        ...
        set(AVG.hFilter.hMainGrid, 'ColumnSizes',[-2 -1],'RowSizes', 19*ones(1,13) );
        ...
        set(AVG.hFilter.hMainVBox, 'Sizes',[-1 30]);
        %% Construct analysis GUI
        AVG.hAnalysis.hMainVbox= uiextras.VBox( 'Parent', AVG.hAnalysis.hMainPanel, 'Spacing',5 );
        AVG.hAnalysis.hMainGrid= uiextras.Grid( 'Parent', AVG.hAnalysis.hMainVbox, 'Spacing',10 );        
        ...
        set(AVG.hAnalysis.hMainGrid,'ColumnSizes',[-1 -1]);
        ...
        set(AVG.hAnalysis.hMainVbox,'Sizes',[-1 40]);
        %% Construct Trigger Box
        AVG.hTrigger.MainVBox=uiextras.VBox('Parent', AVG.hTrigger.hMainTriggerPanel, 'Padding', 4, 'Spacing', 4);
        
        AVG.hTrigger.navigationHBox=uiextras.HBox('Parent', AVG.hTrigger.MainVBox, 'Padding', 4, 'Spacing', 4);
        ...
        set(AVG.hTrigger.navigationHBox, 'Sizes',[-1 40 -1]);
        
        AVG.hTrigger.hGetTrigFromRawFiles=uicontrol('Parent', AVG.hTrigger.MainVBox, 'HorizontalAlignment','left','Style','check', 'String','load trig. data','value',true);
        
        AVG.hTrigger.OffsetHBox=uiextras.HBox('Parent', AVG.hTrigger.MainVBox, 'Padding', 4, 'Spacing', 4);
        ...
        set(AVG.hTrigger.OffsetHBox, 'Sizes',[-1 -2]);
        
        AVG.hTrigger.selectSubPopulationHBox=uiextras.HBox('Parent', AVG.hTrigger.MainVBox, 'Padding', 4, 'Spacing', 4);
        ...
        set(AVG.hTrigger.selectSubPopulationHBox, 'Sizes',[-1 -2]);
        
        AVG.hTrigger.manualSetHBox=uiextras.HBox('Parent', AVG.hTrigger.MainVBox, 'Padding', 4, 'Spacing', 4);
        ...
        set(AVG.hTrigger.manualSetHBox, 'Sizes',[-1 -2]);
        
        AVG.hTrigger.manualLoadHBox=uiextras.HBox('Parent', AVG.hTrigger.MainVBox, 'Padding', 4, 'Spacing', 4);
        ...
        AVG.hTrigger.exportTriggerHBox=uiextras.HBox('Parent', AVG.hTrigger.MainVBox, 'Padding', 4, 'Spacing', 4);
        ...
        AVG.hTrigger.MainGrid=uiextras.Grid('Parent',AVG.hTrigger.MainVBox, 'Spacing',4, 'Padding',4);
        
        set(AVG.hTrigger.MainVBox, 'Sizes',[50 25 25 25 25 25 50 25 -1]);
    end %createGUI
%% %%%%%%%%%%%%%%%%%%%%%%%%%% Create Trigger GUI %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function createTriggerGUI() %the function sets the first trigger to be default
        ...
        set(AVG.hTrigger.MainGrid, 'ColumnSizes', [-1 -1 -1], 'RowSizes', 20*ones(1,min(AVG.Params.maxNumberOfTriggerInColumn,AVG.Params.nTriggers)));
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end %EOF
