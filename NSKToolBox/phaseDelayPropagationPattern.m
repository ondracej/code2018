function [delaysMean_ms,medianCC,h,hCbar,hSingleTracesPlot,delaysAll_ms,allCC]=phaseDelayPropagationPattern(M,Fs,ch,En,varargin)
% [delaysMean_ms,medianCC,h,hSingleTracesPlot,delaysAll_ms,allCCFiltered,allCC]=lowFreqPropagationPattern(M,Fs,ch,En,varargin)
...
for j=1:nTrials
    ...
    for k=1:nCh
        tmpX(k,:)=xcorr(M(k,j,:),M(refernceCh,j,:),maxLag,'coeff');
    end
