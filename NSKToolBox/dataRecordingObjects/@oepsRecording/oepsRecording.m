classdef oepsRecording < dataRecording
    properties
        
        format
        version
        header_bytes
        %samplingFrequency %Cannot define property 'samplingFrequency' in class 'oepsRecording' because the property has already been defined in the superclass 'dataRecording'.
        sample_us
        description
        date_created
        channel
    ...
    methods
        ...
        function [V_uV,t_ms]=getData(obj,channels,startTime_ms,window_ms)
            ...
            %obj.convertData2Double = 0; %%%%%%%%%% Janie
        ...
        function [T_ms,chNumber,chName]=getTrigger(obj,startTime_ms,window_ms,name)
            ...
            %Bits are coded as decimal integers
             ...
             Config.ReqFields = [1 1 1 1 1];
    % Initial setting of extraction mode:
    Config.ReqHeader = 1;   % Initially, include header extraction.
    Config.ExtractMode = 1; % Initially, extract the full length of data.
    Config.ModeArray = [];
    
    %% Call NlxMatEV.m and extract all events of the specified cheetah session
    
    %Config.InDataDir = cscPaths{j};
    Config.InDataDir = obj.recordingDir;
    Config.EventFile = strcat(Config.InDataDir, 'Events.nev');
        ...
        function obj=getHeaderInformation(obj) %get parameters from file header
            disp('Extracting header information');
            ...
            for i=1:numel(obj.channelNumbers)
                ...
                hdr = fread(obj.fid(i), NUM_HEADER_BYTES, 'char*1');
                eval(char(hdr'));
                info.header = header;
                ...
                strParts=fieldnames(header);
                
                for o = 1:numel(strParts)
                    
                    switch(strParts{o})
                        case 'format'
                            obj.format{i}=header.format;
                        case 'version'
                            obj.version(i)=header.version;
                        case 'header_bytes'
                            obj.header_bytes(i)=header.header_bytes;
                        case 'sampleRate'
                            obj.samplingFrequency(i)=header.sampleRate;
                            obj.sample_us(i)=1/obj.samplingFrequency(i)*1e6;
                        case 'description'
                            obj.description{i}=header.description;
                        case 'date_created'
                            obj.date_created{i}=header.date_created;
                        case 'channel'
                            obj.channel(i)=str2num(header.channel(3:end));
                        case 'channelType'
                            obj.channelType{i}=header.channelType;
                        case 'blockLength'
                            obj.blockLength(i)=header.blockLength;
                        case 'bufferSize'
                            obj.bufferSize(i)=header.bufferSize;
                        case 'bitVolts'
                            obj.bitVolts(i)=header.bitVolts;
                    end
                end
...
%                             obj.startDate{i}=strParts{4:7};
%                         elseif strParts{3}(1)=='C' %'Closed'
%                             obj.endDate{i}=strParts{4:7};
%                         end
%                     case '-RecordSize'
...
%             obj.ZeroADValue=0;
%             obj.MicrovoltsPerAD=obj.ADBitVolts(1)*1e6; %% Do not comment this out
disp('')
        end
        ...
        function obj=extractMetaData(obj)
            ...
            [data, timestamps, info] = load_open_ephys_data([obj.recordingDir '\' obj.channelFiles{1}]);
            
            obj.nRecords=numel(info.recNum);
            obj.recordLength_s=timestamps(end);
            
            %fseek(obj.fid(1), obj.headerSizeByte, 'bof');
            %obj.allTimeStamps_us=fread(obj.fid(1), obj.nRecords, 'int64',obj.recordSize(1)-64/8);
            %obj.allTimeStamps_s=timestamps;
            obj.recordingStartTime_s=timestamps(1);
            obj.allTimeStamps_s= timestamps(1:end) - timestamps(1);
            obj.recordingDuration_s=obj.allTimeStamps_s(end);
            obj.recordingDuration_ms=obj.allTimeStamps_s*1000;
            ...
            triggerNames=dir([obj.recordingDir filesep '*.events']);
    ...
    methods (Hidden)
        %class constructor
        function obj = oepsRecording(recordingFile)
            ...
            obj.channelNumbers=cell2mat(cellfun(@(x) str2double(x(7:end-11)),obj.channelFiles,'UniformOutput',0));
