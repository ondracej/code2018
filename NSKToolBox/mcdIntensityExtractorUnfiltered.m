% [IA,tA,icA,trigger]=mcdIntensityExtractorUnfiltered(recObj,SelectedChannels,Bin,StdAbsNoiseConstant)
...
paddingSamples=F.getNumberOfRequiredPaddingSamples;
padding_ms=paddingSamples/samplingFrequency*1000;

Bin=Bin_ms*(samplingFrequency/1000);
...
M=squeeze(F.getFilteredData(recObj.getData(SelectedChannels,0,Tstep+2*padding_ms)));
for i=1:nCh
    Mtmp=M(i,paddingSamples:end-paddingSamples-1);
...
for j=1:nWin
    ...
    M=squeeze(F.getFilteredData(recObj.getData(validCh,startTimes(j)-padding_ms,Tstep+padding_ms*2)));
    M=M(:,(paddingSamples+1):(end-paddingSamples));
    Times=startTimes(j)+((Bin_ms/2):Bin_ms:(size(M,2)/samplingFrequency*1000));
    for k=1:nValidCh
        Mtmp=M(k,:);
        [WinLen NWindows]=size(Mtmp);
...
Times=cumDuration+((Bin_ms/2):Bin_ms:(size(M,2)/samplingFrequency*1000));
for k=1:nValidCh
    Mtmp=M(k,:);
    [WinLen NWindows]=size(Mtmp);
    %Activity intensity calculation
...
for i=1:nValidCh
    ...
    P=uint32(find(diff(tA_tmp) >= (Bin/2/(samplingFrequency/1000)) ));
