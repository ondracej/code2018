...
%[hE,hB]=myErrorBar(x,y,dy,varargin)
%width [0-1] - the width in normalize units of the relative fraction of the bar (e.g. width=1 -> error bar in size of bar
%[h,hE,hB]=myErrorBar(1:8,Int,Int+IntStd,0.4,'Bfacecolor','BLinewidth',5,[0.3 0.3 0.3],'Ecolor','k');
if isempty(width)
    width=0.25;
end
if isempty(x)
...
hhE=get(hE,'children');		
xD = get(hhE(2),'xdata');		% Get xdata from errorbar plot
