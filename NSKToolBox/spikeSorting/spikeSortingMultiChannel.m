...
gridSize=3; %must be an odd number. If empty extracts using all channels
gridExt=1;
...
if ~isempty(gridSizeExt)
    ...
    for i=1:nCh
        ...
        surCh=EnExt(x-gridExtension:x+gridExtension,y-gridExtension:y+gridExtension);
        ...
        surChOverlap=EnExt(x-gridExt:x+gridExt,y-gridExt:y+gridExt);
        surChOverlap(gridExt+1,gridExt+1)=NaN;
        ...
        chPar.surChExt{i}=EnExt(x-overheadGridExtension:x+overheadGridExtension,y-overheadGridExtension:y+overheadGridExtension);
