...
if isempty(WF) %run on artificial data
    ...
    tV0=V0*ones(1,nNeurons);
    %tL=L0+rand(1,nNeurons)*50-25;
    tL=L0*ones(1,nNeurons);
...
% find spike extremum ch
if useSpikeExtremum
    [vMaxAll,pMaxAll]=max(abs(WF),[],3);
    [vMax,pMax]=max(vMaxAll,[],2);
else
    [vMaxAll,pMaxAll]=min(WF,[],3);
    [vMax,pMax]=min(vMaxAll,[],2);
    vMax=abs(vMax);
    vMaxAll=abs(vMaxAll);
end
...
if isempty(est) %if no previous estimation was entered
    ...
    Z=0:dZ:maxZ;
    [Xshifts,Yshifts,Zshifts]=meshgrid(X,Y,Z);
    ...
    for i=1:nNeurons
        ...
        pXY=sqrt((Xc-Xc(pMax(i))).^2+(Yc-Yc(pMax(i))).^2)<maxDistForElectrodes;
        xTmp=Xc(pXY)-Xc(pMax(i));
        yTmp=Yc(pXY)-Yc(pMax(i));
        zTmp=zeros(size(yTmp));
        V0tmp=vMaxAll(i,pXY);
        
        if strcmp(modelType,'globalMinima')
            clear V0; %changes length in loop -> needs to be reset
            
            %clear a 4-D matrix with all possible value within the given parameter range and increments
            V0(1,1,1,:)=vMax(i):dV0:maxV0;
            V0All=bsxfun(@times,V0,ones(size(Zshifts)));
            XAll=bsxfun(@times,Xshifts,ones(size(V0)));
            YAll=bsxfun(@times,Yshifts,ones(size(V0)));
            ZAll=bsxfun(@times,Zshifts,ones(size(V0)));
            
            score=zeros(size(V0All));
            for k=1:numel(xTmp)
                score=score+(       V0tmp(k)-V0All.*exp(-sqrt((xTmp(k)-XAll).^2+(yTmp(k)-YAll).^2+(zTmp(k)-ZAll).^2)./L0)         ).^2;
            ...
            [bestXp,bestYp,bestZp,bestV0p]=ind2sub(size(V0All),bestPlace);
            est.X(i)=X(bestXp);
            est.Y(i)=Y(bestYp);
            est.Z(i)=Z(bestZp);
            est.V0(i)=V0(bestV0p);
        ...
        elseif strcmp(modelType,'reducedModelOpt')
            ...
            for j=1:numel(LGrid)
                [estTmpTmp,eValTmp] = fminsearch(@(x) XYZLV_Model(x,xTmp,yTmp,zTmp,V0tmp,mxV0),[0,0,0,LGrid(j),mxV0],optimset('MaxFunEvals',1000000,'Display','off'));
                %[estTmpTmp,eValTmp] = fminsearch(@(x) XYZL_Model(x,xTmp,yTmp,zTmp,V0tmp,V0),[0,0,0,LGrid(j)],optimset('MaxFunEvals',100000));
                %estTmp = fminsearch(@(x) XYZV_Model(x,xTmp,yTmp,zTmp,V0tmp,L0,vMax(i)),[0,0,0,vMax(i)],optimset('MaxFunEvals',1000000));
                %estTmp = fminsearch(@(x) XYV_Model(x,xTmp,yTmp,V0tmp,L0,vMax(i)),[0,0,vMax(i)],optimset('MaxFunEvals',1000000));
                if eValTmp<eVal
                    estTmp=estTmpTmp;
                    eVal=eValTmp;
                end
        ...
        est.Xrs(i)=est.X(i)+Xc(pMax(i));
        est.Yrs(i)=est.Y(i)+Yc(pMax(i));
        est.Zrs(i)=est.Z(i);
        est.electrodeX=Xc;
        est.electrodeY=Yc;
    end
...
if plotTriangulationOnWaveforms
    ...
    for i=1:nNeurons
        ...
        if plot1NeuronPerPlotInTriangulation
            localGrid=En;
            [yLoc,xLoc]=find(localGrid==pMax(i));
        else
            [x,y]=find(EnExt==pMax(i));
        ...
        %plot( (Xc(tmpCh)-Xc(pMax(i)) )/electrodePitch + arrayExt + 1 , (Yc(tmpCh)-Yc(pMax(i)) )/electrodePitch + arrayExt + 1 ,'o','color',[0.8 0.8 0.8]);hold on;
        ...
        %plot( (est.Xrs(i)-Xc(pMax(i)) )/electrodePitch + arrayExt + 1 , (est.Yrs(i)-Yc(pMax(i)) )/electrodePitch + arrayExt + 1 ,'o','color',[0.9 0.2 0.2],'lineWidth',3);
        
        if plotSpheres
            surface((est.Xrs(i)+Xsp-Xc(pMax(i)) )/electrodePitch + xLoc,...
                (est.Yrs(i)+Ysp-Yc(pMax(i)) )/electrodePitch + yLoc,...
                (est.Zrs(i)+Zsp)/electrodePitch,'FaceColor',cMap(round(normZ(i)*63)+1,:,:),'EdgeColor','none','FaceAlpha',0.4);
            camlight left;
        else
            hold on;plot((est.Xrs(i)-Xc(pMax(i)) )/electrodePitch + xLoc,(est.Yrs(i)-Yc(pMax(i)) )/electrodePitch + yLoc,'o','MarkerSize',8,'Color',[0.3 0.2 0.8]);
        end
