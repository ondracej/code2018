...
smartInterSmoothness=0.000002;
weightFunctionStdMs=7;
maxPostSpikeWidthMs=2;
stdThresholdCrossingSpikeInitiation=3;
    ...
    switch preProcessing
        ...
        case 'smartInterpolation'
            ...
            for i=1:nNeurons
                ...
                %spikeInitiationWF=squeeze(avgWF(i,pMaxSpikeElec(i),(preSpikeSamples-preSpikePeakSamples):preSpikeSamples));
                stdProfile=std(spikeInitiationWF);
                pTransition=min([preSpikePeakSamples-preSpikeMaxInitiationSamples,find(stdProfile > mean(stdProfile(pBaseline)) + stdThresholdCrossingSpikeInitiation*std(stdProfile(pBaseline)),1,'first')-1]);
                w=ones(1,nSamples);
                pSpikeStart=(preSpikeSamples-preSpikePeakSamples+pTransition);
                pSpikeSoftEnd=(preSpikeSamples+maxPostSpikeWidthSamples);
                w(pSpikeStart : pSpikeSoftEnd)=0;
                w((pSpikeSoftEnd+1):(pSpikeSoftEnd+weightFunctionStdSamples*3))=1-exp(-( (1:weightFunctionStdSamples*3)/weightFunctionStdSamples).^2);
                lowpassWF(i,:,:) = csaps(1:nSamples,squeeze(avgWF(i,:,:)),smartInterSmoothness,1:nSamples,w);
            end
...
if strcmp(maxSIFMethod,'spikeNN')
    ...
    for i=1:nNeurons
        ...
        [~,pMaxField(i)]=max(max(abs(lowpassWF(i,pElecs,(preSpikeSamples+postSpikeFieldStartSamples):(preSpikeSamples+postSpikeFieldEndSamples))),[],3),[],2);
        maxChWF(i,:)=lowpassWF(i,pMaxField(i),:);
    end
...
if any(toClassify)
    ...
    fieldPar.classIE=zeros(1,nNeurons);
...
if plotMaxWFAll
    ...
    for i=1:nNeurons
        ...
        plot(squeeze(avgWF(i,pMaxElec(i),:)));hold on;
        plot(squeeze(lowpassWF(i,pMaxElec(i),:)),'r');axis tight;
        set(h,'XTickLabel',[],'YTickLabel',[],'XColor',cMap(4-fieldPar.classIE(i),:),'YColor',cMap(4-fieldPar.classIE(i),:));
        %text(h.XLim(2),h.YLim(2),[num2str(neuronNames(1,i)) '-' num2str(neuronNames(2,i))],'VerticalAlignment','top','HorizontalAlignment','right');
        text(h.XLim(2),h.YLim(2),num2str(i),'VerticalAlignment','top','HorizontalAlignment','right');
    end
end
%{
        for i=1:nNeurons
...
switch PSFMethod
    case 'max'
        ...
        %{
        for i=1:nNeurons;
        h1=subplot(3,4,[1 11]);[hPlot,scaleFac]=activityTracePhysicalSpacePlot(h1,1:120,squeeze(avgWF(i,:,:)),En,'traceColor','r');hold on;
        activityTracePhysicalSpacePlot(h1,1:120,squeeze(lowpassWF(i,:,:)),En,'scaleFac',scaleFac,'DrawElectrodeNumbers',1);title(['Max ch=' num2str(pMaxElec(i))]);
        h2=subplot(3,4,8);hCB=IntensityPhysicalSpacePlot(ch,fieldPar.val(i,:),En,'h',h2,'plotElectrodeNumbers',0);title(num2str(fieldPar.classIE(i)));
        pause;
        delete([h1 h2]);
        end
        %}
