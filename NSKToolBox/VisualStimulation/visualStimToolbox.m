...
VS.par.VSMethods=dir([VS.par.VSDirectory VS.par.dirSep 'VS_*.m']);
...
if str2num(matlabVer{1})>=8 && str2num(matlabVer{2})>=4
...
[VS.par.VSObjDir, name, ext] = fileparts(tmpDir);

tmpFiles=dir([VS.par.VSObjDir VS.par.dirSep 'VS_*.m']);
tmpFiles={tmpFiles.name};
VS.par.VSObjNames=cellfun(@(x) x(1:end-2),tmpFiles,'UniformOutput',0);

VS.par.currentVSO=find(strcmp(VS.par.VSObjNames,'VS_testStim')); %the default visual stim (first on the list)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function initializeVisualStim()
        ...
        eval(['VS.par.VSO=' VS.par.VSObjNames{VS.par.currentVSO} '(VS.par.PTB_win,VS.hand.GenealBox.hInteractiveGUIparent);']);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function CallbackChangeVisualStim(hObj,event,selectedVSO)
        set(VS.hand.visualStimMenu.([VS.par.VSObjNames{VS.par.currentVSO}]),'Checked','off');
        set(VS.hand.visualStimMenu.([VS.par.VSObjNames{selectedVSO}]),'Checked','on'); %select one of the stims
        ...
        if strcmp(props.VS_class,class(VS.par.VSO))
            for i=1:size(props.props,1)
                if isprop(VS.par.VSO,props.props{i,1})
                    VS.par.VSO.(props.props{i,1})=props.props{i,2};
                else
    ...
    function CallbackFileMenuSaveParams(hObj,event)
        ...
        props(:,1)=VS.par.VSOProp;
        props(:,2)=VS.par.VSOPropVal;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function createVSGUI()
        ...
        for i=1:length(VS.par.VSObjNames)
            VS.hand.visualStimMenu.([VS.par.VSObjNames{i}])=uimenu(VS.hand.hVisualStimMenu,...
                'Label', VS.par.VSObjNames{i}, 'Checked','off', 'Callback', {@CallbackChangeVisualStim,i});
        end
        set(VS.hand.visualStimMenu.([VS.par.VSObjNames{VS.par.currentVSO}]),'Checked','on'); %select one of the stims
