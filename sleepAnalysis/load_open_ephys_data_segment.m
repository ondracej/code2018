function [data, timestamps, info] = load_open_ephys_data_segment(filename, t_start_s, dur_s)
...
%% Define the indexes by converting from ms

fs = header.sampleRate;

index_tmp = t_start_s*fs;
index_tmp = floor(index_tmp/SAMPLES_PER_RECORD);
ind_dur = dur_s*fs;
ind_dur  = ceil(ind_dur/SAMPLES_PER_RECORD);
...
elseif strcmp(filetype, 'continuous')
    ...
    for o = 1:ind_dur
        ...
        while ftell(fid) + RECORD_SIZE <= filesize % at least one record remains
            ...
            if ~go_back_to_start_of_loop
                ...
                info.ts(cnt) = timestamp;
                info.nsamples(cnt) = nsamples;
                
                if version >= 0.2
                    info.recNum(cnt) = recNum;
                end
    ...
    timestamps = 1:numel(data);
    timestamps_s = timestamps/fs;
    figure; plot(timestamps_s, data)
